...
class OnPolicyAlgorithm(BaseAlgorithm):
    ...
    def dump_logs(self, iteration: int = 0) -> None:
        ...
        if len(self.ep_success_buffer) > 0:
            ...
            if self.ep_collision_buffer is not None and len(self.ep_collision_buffer) > 0:
                self.logger.record("rollout/collision_rate", safe_mean(self.ep_collision_buffer))
            if self.ep_out_of_bound_buffer is not None and len(self.ep_out_of_bound_buffer) > 0:
                self.logger.record("rollout/out_of_bound_rate", safe_mean(self.ep_out_of_bound_buffer))
            if self.ep_timeout_buffer is not None and len(self.ep_timeout_buffer) > 0:
                self.logger.record("rollout/timeout_rate", safe_mean(self.ep_timeout_buffer))
            if self.ep_success_len_buffer is not None and len(self.ep_success_len_buffer) > 0:
                self.logger.record("rollout/success_len_mean", safe_mean(self.ep_success_len_buffer))
            if self.ep_success_dist_buffer is not None and len(self.ep_success_dist_buffer) > 0:
